...
class WpHelper
{
    ...
    public function postmeta($post_id, $meta_key, $fallback = false)
    {
        if (isset($this->postmeta[$post_id])) {
            ...
            if ($this->postmeta['all']->where('post_id', '=', $post_id)->where('meta_key', '=', $meta_key)->first()) {
                return $this->postmeta['all']->where('post_id', '=', $post_id)
                    ->where('meta_key', '=', $meta_key)->first()->meta_value;
            }
        } else {
            $postmeta = DB::table($this->table_prefix.'postmeta')
                ->select('meta_value')
    ...
    public function allPostmeta($post_id = false)
    {
        if ($post_id) {
            if (!isset($this->postmeta[$post_id])) {
                $this->postmeta[$post_id] = DB::table($this->table_prefix.'postmeta')
